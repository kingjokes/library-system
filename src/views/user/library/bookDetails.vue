...
const borrowedStatus = computed(() =>
  borrowedBooks.value.find((item) => item.book_id == route.params.id) || {status:''}
);
...
const borrowBook = async () => {
  ...
  await userStore
    .submitBookRequest({
      status: userStore.bookDetails?.status === 0 ? 0 : 3,
      user_id: userStore.user?.id,
      request_date: new Date().toISOString().slice(0, 19).replace("T", " "),
      book_id: userStore.bookDetails.id,
      pickup: selectedOption.value,
      due_date: new Date(
        addDays(new Date(), userStore.user?.role === "student" ? 7 : 14)
      )
        .toISOString()
        .slice(0, 19)
        .replace("T", " "),
    ...
    .then(async (response) => {
      ...
      if (response.status) {
        dialog.value = true;
        userStore.fetchBookDetails(route.params.id);
    ...
    <v-row>
      ...
      <v-col cols="12" md="5" sm="5" lg="5">
        ...
        <v-btn
          ...
          width="200"
        >
          <span> Pending Book Request</span>
        </v-btn>
        <v-btn
          v-else-if="borrowedStatus.status === 1"
          disabled
          color="#000"
          class="text-white text-capitalize"
          flat
          width="200"
        >
          <span> Book Already Borrowed</span>
        </v-btn>
        <v-btn
          v-else-if="borrowedStatus.status === 4"
          disabled
          color="#000"
          class="text-white text-capitalize"
          flat
          width="200"
        >
          <span> Pending Book Return</span>
        </v-btn>

        <v-btn
          v-else-if="borrowedStatus.status === 3"
          disabled
          color="#000"
          class="text-white text-capitalize"
          flat
          width="200"
        >
          <span> Book already Reserved</span>
        </v-btn>
